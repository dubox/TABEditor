...
const props = defineProps({
  type: { type: String, default: 'single' },// single单独 ，union 组合
  direction: { type: String, default: 'left' },//left center right
...
<template>
  ...
  <div v-if="type == 'union'"
    :class="` note-flag absolute bottom-0 ${direction == 'center' ? 'w-full' : 'w-1/2'} ${direction == 'center' ? 'left' : direction}-0`">
    <LineH v-if="duration >= 32" :lineWidth="2" class=" mt-1"></LineH>
  ...
  <template v-if="type == 'single'">
    ...
    <svg v-if="duration > 4" width="10.5" height="38.5" class=" note-flag absolute bottom-0 left-1/2">
